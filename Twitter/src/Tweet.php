...
class Tweet
{
     ...
     * 
     */
    public static function getRetweetsData($id_user){
        try{
            $pdo = Database::connect();
            $query = "SELECT tweet.id_user as 'id_user_of_retweet', user.*, tweet.* from retweet INNER JOIN tweet ON retweet.id_tweet=tweet.id INNER JOIN user ON tweet.id_user=user.id WHERE retweet.id_user= :id_user;";
            $smt = $pdo->prepare($query);
